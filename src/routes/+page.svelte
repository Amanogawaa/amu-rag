<script lang="ts">
	...
	import { findRelevantEntries, formatContext, generateFlexibleAnswer } from '$lib/data/data.me';
		...
		try {
			...
			while (true) {
				...
				messages = messages.map((msg, i) =>
					i === messages.length - 1 ? { ...msg, content: accumulatedResponse } : msg
				);
			}
...
>
	<div
		class="max-w-3xl max-h-[487px] h-full overflow-y-scroll mx-auto w-full flex-grow flex flex-col justify-center gap-5"
	>
		{#if messages.length === 0}
			<div class="w-full text-center text-3xl">
				{loading ? 'witaminit kapeng mainit...' : 'wilcom back! im miria, your personal assistant?'}
			</div>
		{:else}
			...
			{#if loading}
				<div class="chat chat-start w-full">
					<div class="w-full text-justify text-base">witaminit kapeng mainit...</div>
				</div>
			{/if}
